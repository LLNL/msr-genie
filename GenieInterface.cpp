...
void get_df_dm(GenieDataManager &manager)
{
    ...
    std::cout << "number of dfdms associated with MSR " << in << ": " <<
...
void get_bitmask(GenieDataManager &manager)
{
    ...
    cout << "MSR address: " << setw(18) << ret[0] << setw(18) << "\tMASK: " <<
         ret[1] << "\t Table: " << ret[2] << "\n";
}
...
void help()
{
    ...
    cout << "Debug options dumps all MSR data currently stored in memory. First block is a list containing \n"
         ...
         << "To check your machine's df_dm (Intel), run \"lscpu\" in their terminal and convert the values\n"
